# PCBForge workspace scaffold (playbook §2.1 stand-in)

> Provenance: this file was authored by the coding agent at the operator's
> direction (2026-07-08) because the original playbook §2.1 content was never
> pasted in. It is now the normative scaffold: INF-1 executes it verbatim.
...
├── crates/
│   ├── core/             # shared types only — no deps, no I/O
│   ├── ingest/           # KiCad/Gerber/Excellon → core types
│   ├── cam/              # geometry kernel + ablation CAM + emitters
│   ├── vision/           # camera, calibration, classification
│   ├── orchestra/        # persistence, stage engine, interlocks
│   ├── drivers/          # Marker trait + sim/native backends
│   ├── testkit/          # golden-image test harness (dev-dep only)
│   └── cli/              # the `pcbforge` binary
```

`xtask/` is added later by INF-3; driver crates (`driver-jcz`) are added by
DRV-3. Do not create them now.

## 2. Commands (run from the repo root, verbatim)

```sh
cargo new --lib crates/core --name core
cargo new --lib crates/ingest --name ingest
cargo new --lib crates/cam --name cam
cargo new --lib crates/vision --name vision
cargo new --lib crates/orchestra --name orchestra
cargo new --lib crates/drivers --name drivers
cargo new --lib crates/testkit --name testkit
cargo new --bin crates/cli --name pcbforge
```

Root `Cargo.toml` (write exactly):
...
```sh
cargo add -p cam i-overlay
cargo add -p cam cavalier_contours
cargo add -p vision nalgebra
```

Commit `Cargo.lock`.

## 3. `crates/core/src/lib.rs` (verbatim)
...
Each other crate's `lib.rs` is initially empty except a crate-level doc
comment naming its workstream (e.g. `//! WS-CAM — geometry kernel & ablation
CAM.`). `crates/cli/src/main.rs` stays the default hello-world until EMIT-3.
Later tasks fill the crates in.

## 5. Done-when (INF-1)

- `cargo build` and `cargo test` succeed at the workspace root.
- `cargo fmt --check` passes.
- `Cargo.lock` committed; `git status` clean after commit.
- No dependency added beyond: i-overlay, cavalier_contours (cam), nalgebra
  (vision).
